...
async def context(
    ...
    A browser context is like an incognito session - it has its own
    cookies, cache, and local storage. This ensures test isolation.

    Args:
        browser: Browser instance from playwright-pytest plugin
        browser_context_args: Context configuration arguments
    ...
    """
    # Create context with configured arguments
    context = await browser.new_context(**browser_context_args)

    # Enable tracing if configured
    if settings.test.trace_mode != "off":
    ...
    if settings.test.trace_mode != "off":
        trace_dir = settings.reports_dir / "traces"
        trace_dir.mkdir(parents=True, exist_ok=True)

        trace_path = trace_dir / f"trace_{asyncio.current_task().get_name()}.zip"
...
async def page(context: BrowserContext) -> AsyncGenerator[Page, None]:
    """
    Provide a page instance for each test.

    This is the main fixture used in most tests. It provides a fresh page
    with configured timeouts and default behaviors.
    ...
    page.set_default_navigation_timeout(settings.test.page_load_timeout)
    ...
    if settings.test.screenshot_on_failure:
        from utils.helpers import take_screenshot

        # Check if test failed (using stash from pytest_runtest_makereport)
        test_item = None
        try:
            # Access test item from current request
            test_item = pytest.current_test_item  # type: ignore
        except AttributeError:
            pass

        if test_item and phase_key in test_item.stash:
            report = test_item.stash[phase_key].get("report")
            if report and report.failed:
                await take_screenshot(page, f"{test_item.name}_failure", full_page=True)
